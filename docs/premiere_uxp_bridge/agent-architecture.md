...
- **現状**: WSS ブリッジは接続確立済み。公開 CA 証明書で TLS、token 認証、
  allowlist（読み取り 2 メソッド: `project.assets.list`, `sequence.getState`）で
  稼働。動作確認済みツールの詳細は [mcp-tools.md](mcp-tools.md) を参照。
...
| 2 | `program.exportFrame`（L2） | read | 予定 |
| 3 | `sequence.insertClip` / `sequence.addMarker` | write（公式 DOM） | 予定 |
| 4 | `clip.addEffect`（QE DOM） | write（非公式・リスク明記） | 予定 |
